...
export default function Programcilar() {
  ...
  const oneCikaninIsmi = () => {
    ...
    return programcilar.filter(obj => obj.id == enIyi)[0].isim;
  };
  
  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: enIyi ? 'gold' : 'royalblue', // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          programcilar.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {setenIyi(dev.id) /* burada dev.id 'yi öne çıkan id'ye atayın */ }}>Kutla</button>
